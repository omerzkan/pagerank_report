...
Another way to see it: with $\ve = (1,\dots,1)^T$, a matrix $\mathbf{B}$ is column-stochastic exactly when $\mathbf{B}^T\ve = \ve$ and $\mathbf{B} \ge 0$.
Then $(\mathbf{BC})^T\ve = \mathbf{C}^T\mathbf{B}^T\ve = \mathbf{C}^T\ve = \ve$.

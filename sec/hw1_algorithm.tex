...
Following the paper, we do not change $\A$: the dangling columns stay zero, and $\M$ is substochastic.
Its largest eigenvalue is then smaller than $1$, so the solver scales $\vx_k$ to sum $1$ after every step (Section~\ref{sec:implementation}); the dangling test of Exercise~4 in the paper is this situation.

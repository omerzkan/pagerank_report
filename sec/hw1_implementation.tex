...
It starts from $\vx_0 = (1/n,\dots,1/n)^T$ and repeats two steps until $\|\vx_{k+1}-\vx_k\|_1 < \texttt{tol}$:
...
\end{enumerate}
After Step~B we divide $\vx_{k+1}$ by its sum. When there are no dangling nodes the sum is already $1$ in exact arithmetic and this only removes rounding errors; when there are dangling nodes the sum is smaller than $1$ and the division keeps the vector scaled to sum $1$.
The function returns the vector $\vx$, the number of iterations $k$ and the last residual $\|\vx_{k+1}-\vx_k\|_1$.
...
We also tested the solver on a small web with a dangling node (Figure~2.1 without the link $3 \to 1$, so page~3 is dangling): the solver and a dense eigenvector computation of $\M$ give the same vector (difference $1.3\cdot10^{-13}$).
...
\input{tables/hollins_summary}

The most important page is the home page of the university, with a score about 100 times larger than the uniform score $1/n = 0.000166$.
...
\input{tables/hollins_top10}

Every score is at least $m/n = 2.5\cdot10^{-5}$, as formula (3.2) predicts (the smallest score is $2.8\cdot10^{-5}$).